...
RESET_EMBED = (os.getenv("RESET_EMBED", "false").lower() in ("1", "true", "yes"))
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT ln.id,
                       ...
                       ln.ordinal_label AS ordinal_label,
                       COALESCE(ln.heading, ln.title, '') AS heading,
...
def main():
    ...
    if RESET_EMBED:
        try:
            ...
            print("Existing collection 'law_chunks' deleted (RESET_EMBED)")
        except Exception:
            pass
    ...
    ids, embs, docs, metas = [], [], [], []
    for r in rows:
        ordinal = (r.get("ordinal_label") or "").strip()
        heading = (r.get("heading") or "").strip()
        body = (r.get("text") or "").strip()
        parts = [p for p in [ordinal, heading, body] if p]
        txt = ("\n".join(parts)).strip()
        if not txt:
            continue
